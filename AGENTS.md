...
- Do not introduce plugin-owned background goroutines, timers, or flushers except the user-requested reset-follow synchronizer. It runs one serial round every 30 minutes, queries each followed account once, and shares account coordination with page refreshes and manual resets. Reconfiguration, `plugin.quiesce`, and shutdown must stop and join it before switching or closing storage; never abandon in-flight host calls to simulate a timeout. CPA v7.2.143, v7.3.15, and v7.3.17 do not notify plugins when disabled: expose `pause_reset_follow_sync`, document that it must be enabled before disabling the plugin, and do not claim that host disablement alone stops this task. All other work remains synchronous within host calls.
