...
- CPA 无法拆分为输入和输出的用量（例如 CPA v7.3.17 的 Meta 和 Devin 服务商）按 $0 记录，不计入金额额度；Token 总数可信时仍计入 Token 额度。如需限制这类用量，请使用 Token 额度或路由规则。

### 后端

- 出现上述按 $0 记录的用量时，插件日志对每个服务商记录一条错误，包含服务商、模型和 Token 数。

### 前端

- 管理员的请求事件列表中，这类请求在成本下方显示「未计费」标签，悬停可查看原因；API Key 用户页不显示。
...
- 插件改用 Go 1.26 构建，修复 Go 1.24 标准库中 TLS、证书校验、HTTP 和 URL 解析的安全问题，建议尽快升级。macOS 需 12 或更高版本，与当前 CPA 的要求一致。
- 升级后首次启动会删除没有 Key 跟随的账号的额度快照，不影响用量、额度和跟随配置。

### 后端

- Codex 手动重置：上游未确认的重置不再留下记录，成功的记录保留 7 天。API Key 用户对同一账号每分钟最多发起一次新的重置，超出时返回 429 并提示等待秒数；重试已成功的重置不受限制。
- 额度快照只为有 Key 跟随的账号保存；查询 Kimi、xAI、Antigravity 账号的额度不再保存快照。
- API Key 用户刷新订阅页时，1 分钟内已查询过或确认不可用的跟随账号不再重复检查。
- 新增 `reference_price_proxy` 配置，为 models.dev 参考价下载指定代理：留空沿用环境变量，`direct` / `none` 表示直连，也可填写 HTTP、HTTPS、SOCKS5、SOCKS5H 代理地址。
